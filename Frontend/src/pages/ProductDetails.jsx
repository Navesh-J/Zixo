...
function ProductDetails() {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-40">
      ...
      <span className="font-cyber text-[10px] tracking-[0.4em] uppercase text-zinc-500 animate-pulse">Scanning_Artifact...</span>
    </div>
  );
  ...
  if (error) return (
    <div className="py-20 text-center">
      ...
      <p className="font-heading text-xl text-goth-blood uppercase tracking-widest">{error}</p>
      <button onClick={() => navigate("/")} className="mt-6 text-zinc-500 font-cyber text-xs hover:text-white transition-colors uppercase">Return_To_Safe_Sector</button>
    </div>
  );
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      >
        ...
        <span className="font-cyber text-[10px] tracking-widest uppercase">Go_Back</span>
      </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        <div className="relative group">
          ...
          <div className="relative aspect-square bg-goth-void border border-goth-steel flex items-center justify-center overflow-hidden">
            ...
            <img 
              src={product.imageUrl || "https://via.placeholder.com/600?text=NO_IMAGE_FEED"} 
              alt={product.productName}
              className="h-full w-full object-cover grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all duration-1000"
            />
        ...
        <div className="space-y-8">
          <header className="border-b border-goth-steel pb-6">
            ...
            <div className="flex items-center gap-3 text-goth-blood">
               <Zap size={16} />
               <span className="font-cyber text-[10px] tracking-[0.3em] uppercase">Auth_Confirmed // Artifact_Class_A</span>
            </div>
          </header>

          <div className="space-y-4">
            <h3 className="font-cyber text-[11px] text-zinc-500 uppercase tracking-widest">Description_Log:</h3>
            <p className="font-cyber text-sm text-zinc-300 leading-relaxed border-l border-goth-steel pl-4 italic">
              // {product.productDescription}
            </p>
          </div>

          <div className="bg-goth-void border border-goth-steel p-6 flex justify-between items-center">
             <div>
                <span className="block font-cyber text-[9px] text-zinc-600 uppercase tracking-widest mb-1">Exchange_Rate</span>
                <span className="font-cyber text-4xl text-white font-bold tracking-tighter">₹ {product.price}</span>
             </div>
             
             <button
                onClick={() => {
                  if (!isAuthenticated) {
                    navigate("/login", { state: { from: `/product/${id}` }, replace: true });
                    return;
                  }
                  addToCart(product);
                }}
              ...
              >
                <span className="relative z-10">{isAuthenticated ? "ACQUIRE_ARTIFACT" : "LOGIN_TO_TRADE"}</span>
