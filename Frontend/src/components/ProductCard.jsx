  ...
  return (
    ...
    >
      ...
      <div className="relative h-48 bg-goth-black overflow-hidden flex items-center justify-center border-b border-goth-steel">
        ...
        <img
          src={product.imageUrl || "https://via.placeholder.com/400?text=VOID_ARTIFACT"}
      ...
      <div className="p-4 space-y-2">
        <div className="flex justify-between items-start">
          ...
          <Zap size={16} className="text-goth-blood group-hover:animate-flicker transition-opacity" />
        </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-goth-steel/30">
          <div className="flex flex-col">
            <span className="text-[9px] text-zinc-600 uppercase tracking-widest font-cyber">Unit_Price</span>
          ...
          <div className="flex items-center gap-2 font-heading text-[10px] tracking-[0.3em] text-zinc-400 group-hover:text-white transition-colors py-1">
            <Eye size={16} className="group-hover:text-goth-blood transition-colors" />
            <span className="hidden sm:inline">ENGAGE_DETAILS</span>
          </div>
        </div>
      </div>

      {/* Scanning Line Animation (Only on hover) */}
      <div className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-10 bg-linear-to-b from-transparent via-goth-blood to-transparent h-[10%] w-full top-[-10%] group-hover:animate-[scanline_2s_linear_infinite]" />
    </motion.div>
  );
}
